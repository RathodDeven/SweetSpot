...
function MetricCard({ title, value, icon: Icon, trend = null }) {
  return (
    <motion.div
    ...
    >
      ...
      {trend && (
        <div className={`flex items-center mt-2 ${trend.positive ? 'text-green-500' : 'text-red-500'}`}>
...
function AllocationHistoryTable() {
  ...
  const filteredData = MOCK_ALLOCATION_HISTORY.filter(item => 
  ...
  const MobileAllocationCard = ({ item }: { item: typeof MOCK_ALLOCATION_HISTORY[0] }) => (
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
          ...
          <div className="flex-1 text-center sm:text-left">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 mt-2">
              <div className="flex items-center space-x-2 text-gray-600">
                ...
                <span className="text-sm">{formatAddress(MOCK_USER_PROFILE.address)}</span>
              </div>
              <div className="flex items-center space-x-2 text-gray-600">
                <Clock className="h-4 w-4" />
                <span className="text-sm">Member since {MOCK_USER_PROFILE.createdAt.toLocaleDateString()}</span>
              </div>
            </div>
          ...
          <div className="text-center">
            <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-purple-100">
              <div className="text-2xl font-bold text-purple-600">{MOCK_USER_PROFILE.reputation}</div>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold">${(MOCK_FINANCIAL_STATS.totalClaimed.usdValue / MOCK_FINANCIAL_STATS.totalTransactions).toFixed(2)}</span>
            </div>
            <div className="flex justify-between items-center">
